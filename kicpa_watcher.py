...
회사 지정 메일제목/파일명 형식 자동 인식
----------------------------------------
일부 공고는 "메일제목: [...]" / "파일제목: [...]" 처럼 제출 형식을 직접
지정한다. fetch_detail()이 상세페이지 텍스트에서 이런 줄을 찾아 그 안의
성명/생년월일/합격연도 같은 토큰을 실제 값(환경변수)으로 치환한 뒤,
detail["custom_subject"] / detail["custom_filename_base"]에 담아 반환한다.
필요한 값이 비어 있으면(예: 생년월일 미등록) 억지로 채우지 않고 기본
제목/파일명으로 안전하게 대체하며, 그 사실을 텔레그램 알림에 남긴다.
또한 "경력증명서와 병합하여 1개 파일로 제출" 같이 우리가 갖고 있지 않은
서류를 새로 합쳐야 하는 요구는 detail["requires_merge"]로 표시해서
자동 지원메일을 건너뛰고 알림만 보낸다 (자체 양식 첨부 감지와 동일 취급).
...
MERGE_ACTION_KEYWORDS = ("병합하여", "병합해서", "병합 후", "합쳐서", "하나의 파일로", "1개의 파일로", "한 개의 파일로")
MERGE_TARGET_DOC_KEYWORDS = ("경력소개서", "경력증명서", "포트폴리오", "자기소개서", "자소서")
...
# 개인정보는 하드코딩하지 않고 GitHub Secrets에서 읽어온다. 아래 4개(생년월일/
# 출생년도/합격연도/합격회차)는 회사가 메일제목·파일명에 이런 정보를 요구할
# 때만 쓰이는 선택 항목이라, 비워둬도 자동화 자체는 정상 작동한다(그 항목이
# 필요한 공고만 기본 제목/파일명으로 대체됨).
APPLICANT_NAME = os.environ.get("APPLICANT_NAME", "")
APPLICANT_BIRTH_DATE = os.environ.get("APPLICANT_BIRTH_DATE", "")  # 예: 19940101
APPLICANT_BIRTH_YEAR = os.environ.get("APPLICANT_BIRTH_YEAR", "") or (
    APPLICANT_BIRTH_DATE[:4] if len(APPLICANT_BIRTH_DATE) >= 4 and APPLICANT_BIRTH_DATE[:4].isdigit() else ""
)
APPLICANT_PASS_YEAR = os.environ.get("APPLICANT_PASS_YEAR", "")  # 예: 2025
APPLICANT_PASS_ROUND = os.environ.get("APPLICANT_PASS_ROUND", "")  # 예: 60
...
def build_custom_name(raw_pattern: str, company: str, position: str) -> tuple[str, list[str]]:
    """회사가 지정한 형식 문자열(raw_pattern) 안의 토큰을 실제 값으로 치환한다.
    (치환결과, 값이 없어서 못 채운 토큰 목록)을 반환한다. 못 채운 토큰이 있으면
    호출부에서 이 결과를 쓰지 않고 기본 제목/파일명으로 대체해야 한다."""
    token_values = [
        ("생년월일", APPLICANT_BIRTH_DATE),
        ("출생년도", APPLICANT_BIRTH_YEAR),
        ("출생연도", APPLICANT_BIRTH_YEAR),
        ("합격연도", APPLICANT_PASS_YEAR),
    ...
    # 긴 토큰부터 치환해야 "합격회차"를 치환하기 전에 "회차"만 먼저 걸려서
    # 잘못 잘리는 일이 없다.
    token_values.sort(key=lambda pair: -len(pair[0]))
...
def sanitize_filename(name: str) -> str:
    """파일명으로 쓸 수 없는 문자를 제거하고, 형식이 대괄호/괄호로 감싸져
    있으면(예: "[2026 ... ]") 그 괄호를 벗겨서 순수 이름만 남긴다."""
    name = name.strip()
...
def fetch_detail(detail_url_tmpl: str, row_id: str, company: str, position: str) -> dict:
    """상세페이지에서 이메일/마감일/첨부파일/회사 지정 제출형식 등을
    best-effort로 뽑는다. company/position은 회사 지정 형식 문자열 안의
    "회사명"/"지원분야" 토큰을 채우는 데 쓰인다."""
    try:
        ...
        # 회사가 메일제목/파일명 형식을 지정했는지 확인하고, 지정했다면
        # 토큰을 실제 값으로 채워본다. 필요한 값이 없으면 그 항목만
        # naming_missing에 남기고 기본 제목/파일명을 쓰도록 비워 둔다.
        naming = extract_naming_requirements(text)
        missing_all: list[str] = []
        if naming.get("subject_pattern"):
            resolved, missing = build_custom_name(naming["subject_pattern"], company, position)
            if missing:
                missing_all.extend(missing)
            else:
                detail["custom_subject"] = resolved
        if naming.get("file_pattern"):
            resolved, missing = build_custom_name(naming["file_pattern"], company, position)
            if missing:
                missing_all.extend(m for m in missing if m not in missing_all)
            else:
                detail["custom_filename_base"] = sanitize_filename(resolved)
        if missing_all:
            detail["naming_missing"] = sorted(set(missing_all))

        # "다른 서류와 병합해서 1개 파일로 제출"류의 요구 감지. 우리는 이력서
        # 파일 하나만 갖고 있어서 병합 자체가 불가능하므로, 자동 지원메일은
        # 건너뛰고 알림만 보내도록 표시한다.
        if any(mk in text for mk in MERGE_ACTION_KEYWORDS) and any(dk in text for dk in MERGE_TARGET_DOC_KEYWORDS):
            detail["requires_merge"] = True
...
def format_message(board: dict, row: dict, detail: dict, is_bump: bool = False) -> str:
    ...
    if detail.get("custom_subject") or detail.get("custom_filename_base"):
        note = "📌 회사 지정 형식 자동 적용됨"
        if detail.get("custom_subject"):
            note += f" / 메일제목: {detail['custom_subject']}"
        if detail.get("custom_filename_base"):
            note += f" / 파일명: {detail['custom_filename_base']}"
        lines.append(note)
    if detail.get("naming_missing"):
        lines.append(
            f"⚠️ 회사 지정 형식을 감지했지만 필요한 정보({', '.join(detail['naming_missing'])})가 "
            "없어 기본 제목/파일명으로 발송했습니다. Secret 등록 후 필요하면 직접 재발송하세요."
        )
    if detail.get("requires_merge"):
        lines.append("")
        lines.append(
            "⚠️ 이력서와 다른 서류(경력증명서 등)를 하나의 파일로 병합해서 제출하라는 요구 감지 - "
            "자동지원 보류, 직접 확인 후 지원해주세요"
        )
...
def send_application_email(row: dict, detail: dict) -> dict | None:
    ...
    실제 회사가 아니라 본인 메일로만 보낸다. detail에 custom_subject /
    custom_filename_base가 있으면 회사가 지정한 형식을 그대로 쓰고, 없으면
    기본 제목·파일명을 쓴다. 실제(TEST_MODE 아닌) 발송이 성공하면 나중에
    답장 확인용으로 쓸 기록(dict)을 반환하고, 그 외에는 None을 반환한다."""
    ...
    subject = detail.get("custom_subject") or EMAIL_SUBJECT_TEMPLATE.format(
        company=row["company"], position=position, applicant=APPLICANT_NAME
    )
    body = EMAIL_BODY_TEMPLATE.format(company=row["company"], position=position, applicant=APPLICANT_NAME)

    filename_base = detail.get("custom_filename_base")
    docx_filename = f"{filename_base}.docx" if filename_base else RESUME_PATH.name
    pdf_filename = f"{filename_base}.pdf" if filename_base else RESUME_PDF_PATH.name
    ...
    if RESUME_PDF_PATH.exists():
        with open(RESUME_PDF_PATH, "rb") as f:
            pdf_part = MIMEApplication(f.read(), Name=pdf_filename, _subtype="pdf")
...
def process_board(board: dict, state: dict) -> None:
    ...
    if not to_process:
        ...
        for row in to_process:
            ...
            position = position_word(row["title"])
            detail = (
                fetch_detail(board["detail_url"], row["id"], row["company"], position) if row.get("id") else {}
            )
            ...
            if detail.get("attachments") or detail.get("requires_merge"):
                reason = "자체 양식 첨부" if detail.get("attachments") else "서류 병합 요구"
                print(f"[INFO] [{board['key']}] #{row['no']} {row['title']} - {reason} 감지, 자동 지원메일 건너뜀.")
            else:
